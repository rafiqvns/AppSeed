...
import { ActivityIndicator, View, NativeModules, LayoutAnimation, Image, TouchableWithoutFeedback, Alert } from 'react-native';
...
export default class Buttons6 extends PureComponent {
  state = {
    value: 0,
    left: 5 * _width,
  }
  ...
  onChange = (value) => {
    ...
    this.setState({ value, left: value == 0 ? 5 * _width : (value - 1) * _width })
    ...
    if (IS_TAB) {
      return (
        <View style={{ flexDirection: 'row', backgroundColor: colors.APP_LIGHT_GRAY_COLOR }}>
          ...
          <TouchableWithoutFeedback disabled={disabled} onPress={() => this.onButtonPress(5)}>
            <View style={styles.buttonView}>
    ...
    return (
      <>
        ...
        <ActionSheet
          ...
          title={'Select value'}
          options={['1', '2', '3', '4', '5', 'N/A', 'Cancel']}
          cancelButtonIndex={6}
          //destructiveButtonIndex={1}
          onPress={(index) => {
            console.log('option7button index', index);
            if (index < 6)
              this.setState({ value: index == 5 ? 0 : index + 1 }, () => {
                if (this.props.onChange && typeof (this.props.onChange) == 'function')
                  this.props.onChange(index == 5 ? 0 : index + 1)
              })
          }}
        />
      </>
