...
import { Option7Buttons } from '.';

export default class Evaluation7OptionField extends PureComponent {
  ...
  render() {
    ...
    return (
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', height: scale(38), paddingHorizontal: scale(20), paddingVertical: scale(5) }}>
          <Text>{title}</Text>
          <Option7Buttons initialValue={value} onChange={onChange} start_time={start_time} end_time={end_time} />
        </View>
    )
  }
