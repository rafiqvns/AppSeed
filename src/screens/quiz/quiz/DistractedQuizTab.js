...
import {distractedQuiz} from '../questions/distractedQuiz';

import {WebView} from 'react-native-webview';
import RNHTMLtoPDF from 'react-native-html-to-pdf';
import Share from 'react-native-share';
import DestructiveQuizPreview from './preview/DestructiveQuizPreview';
class DistractedQuizTab extends React.Component {
  state = {
    start_time: '12:30 AM',
    date: '',
    name: '',
    ...
    checkBoxState: {},
    mounting: true,
    _distractedQuizes: distractedQuiz,
    signature: null,
    correct: null,
    possible: null,
    effective: null,
  ...
  preview = async () => {
    await this.getDistractedQuizAnswers();
    this.setState({showPreview: true});
  };
  getDistractedQuizAnswers = async () => {
    const {
      company_info,
      student_info,
      selected_test,
  ...
  getQuizQuestions = async () => {
    ...
    let res = await apiGet('/api/v1/quizes/distracted_questions/');
    if (res && res.data) {
      // console.log(res.data);
  ...
  submitQuiz = async () => {
    ...
    var resi = await apiPatch(
      `/api/v1/students/${student_info?.id}/tests/${selected_test?.id}/quizes/distracted/`,
      {
    ...
    let res = await apiPost(
      `/api/v1/students/${student_info?.id}/tests/${selected_test?.id}/quizes/distracted/submit-answers/`,
      quizData,
    );
    // if (res && res.data) {
    //   console.log(res.data);
  ...
  getQuizStats = async () => {
    ...
    let res = await apiGet(
      `/api/v1/students/${student_info?.id}/tests/${selected_test?.id}/quizes/distracted/get-answers/`,
    );
    if (res && res.data) {
      // console.log()
      console.log(res.data);
