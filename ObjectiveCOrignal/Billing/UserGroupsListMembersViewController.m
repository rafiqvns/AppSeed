...
@interface UserGroupsListMembersViewController ()
...
@implementation UserGroupsListMembersViewController
...
- (void)viewDidLoad {
    ...
    if (self.isUserGroup) {
        ...
        if (self.isSelectionMode) {
            self.selectBtn = [[UIBarButtonItem alloc] initWithTitle:@"Select"
                                                              style:UIBarButtonItemStylePlain
                                                             target:self
                                                             action:@selector(selectButtonPressed:)];
            self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects:self.editBtn, self.selectBtn, self.reloadBtn, nil];
        } else {
...
-(IBAction)saveButtonPressed:(id)sender {
    ...
    if (self.allowMultipleSelection) {
        if ([self.addDelegate respondsToSelector:@selector(didAddObjects:forKeys:)]) {
            [self.addDelegate didAddObjects:self.selectedUsers forKeys:[NSArray arrayWithObject:self.addDelegateKey]];
        }
    } else {
...
-(IBAction)selectButtonPressed:(id)sender {
    if ([self.addDelegate respondsToSelector:@selector(didAddObjects:forKeys:)]) {
        NSMutableArray *groupAndMembers = [NSMutableArray array];
        [groupAndMembers addObject:self.groupInfo];
        [groupAndMembers addObjectsFromArray:self.itemsFiltered];
        [self.addDelegate didAddObjects:groupAndMembers forKeys:[NSArray arrayWithObject:self.addDelegateKey]];
    }
}
...
-(IBAction)addMemberButtonPressed:(id)sender {
    UserAggregate *usrAgg = [[DataRepository sharedInstance] getUserAggregateAvailablePlus];
    [usrAgg registerForCallback:self];
    [usrAgg getUserGroupsForCompany:self.company];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isUserGroup) {
        if ([self.selectedUsers containsObject:dict]) {
            //self.selectedUsers = [NSMutableArray array];
            [self.selectedUsers removeObject:dict];
            ...
            [self.selectedUsers addObject:dict];
...
-(void)didAddObjects:(NSArray *)objects forKeys:(NSArray *)keys {
    if ([keys containsObject:KeyAddFunctionalGroupMember]) {
        [self addMembersToCurrentFunctionalGroup:objects];
        NSLog(@"");
    }
    if (DEVICE_IS_IPAD) {
        if ([keys containsObject:KeyAddFunctionalGroupMember]) {
...
-(void)addMembersToCurrentFunctionalGroup:(NSArray*)membersInfo {
    UserAggregate *usrAgg = [[DataRepository sharedInstance] getUserAggregateAvailablePlus];
    [usrAgg registerForCallback:self];

    for (NSDictionary *memberInfo in membersInfo) {
        [self addMemberToCurrentFunctionalGroup:memberInfo];
    }
}
