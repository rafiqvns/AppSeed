...
-(IBAction)saveButtonPressed:(id)sender {
    ...
    NSString *urlEndPoint= [NSString stringWithFormat:@"students/%@/notes/", self.student.userId];
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"newNote"]]) {
        [dict setObject:[self.values objectForKey:@"newNote"] forKey:@"note"];
    }
    ...
    [[ServerManager sharedManager] postRequest:token withUrl:urlEndPoint withDict:dict success:^(id responseObject) {
        NSLog(@"resposnseObject = %@", responseObject);
    } failure:^(NSString *failureReason, NSInteger statusCode) {
        NSLog(@"Note saving has been failed...");
    }];
}
