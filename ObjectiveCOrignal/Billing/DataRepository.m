...
@property (readwrite, assign) NSString *authToken;
...

{
    ...
    [params setObject:uName forKey:@"username"];
    [params setObject:uPassword forKey:@"password"];
    
    NSString *post = [NSString stringWithFormat:@"username=%@&password=%@", uName, uPassword];
    ...
    NSString*  requestStr = [NSString stringWithFormat:kAPICSDFormat,
                             aServer, KAdminToken];
...
//    requestStr = [requestStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    NSMutableURLRequest *urlRequest = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:requestStr]];
...
//    [urlRequest addValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    ...
    HttpClientOperation *clientOp = [[HttpClientOperation alloc] initWithRequest:request];
...
- (Aggregate *) getAggregateForClass:(NSString *)rcoClass
{
    ...
    for (Aggregate *agg in [DataRepository sharedInstance].aggregates )
    {
        ...
        if ([HelperSharedManager isCheckNotNULL:agg.rcoObjectClass]) {
