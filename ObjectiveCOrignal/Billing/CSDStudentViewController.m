...
- (void)viewDidLoad {
    ...
    if ([self.user existsOnServerNew]) {
        ...
        if ([self ignoreDriverFields]) {
            ...
            // 09.12.2019
            self.requiredFields = [NSArray arrayWithObjects:@"company", @"firstname", @"surname", @"driverLicenseClass", @"driverLicenseNumber", @"driverLicenseState", @"dOTExpirationDate", nil];
        }
    } else {
    ...
    if ([self.userItemType isEqualToString:kUserTypeStudent]) {
        self.pickerFields = [NSArray arrayWithObjects:@"company", @"driverLicenseState", @"driverLicenseClass", nil];
    } else {
...
-(void)checkExistingRecord {
    ...
    [[ServerManager sharedManager] checkIfStudentExists:token licenseNum:number licenseState:state success:^(id responseObject) {
        ...
        if (results.count > 0) { // User already exists
            
            [self showSimpleMessage:@"User with this license number and state already exists"];
        } else {
...
-(void)_createOrUpdateRecord {
    ...
    if (!self.user) {
        ...
        usr.userType = self.userItemType;
    } else {
        // if the item exists the we should update it
        usr = self.user;
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.userItemType]) {
    ...
    if ([HelperSharedManager isCheckNotNULL:self.company.companyId]) {
        ...
        NSNumber *val = [NSNumber numberWithInteger:[self.company.companyId integerValue]];
        [info setValue:val forKey:@"company"];
    } else {
    ...
    if ([HelperSharedManager isCheckNotNULL:usr.endorsements]) {
        ...
        if ([usr.endorsements isEqual:@"yes"]) {
            [info setValue:[NSNumber numberWithBool:YES] forKey:@"endorsements"];
        } else {
            [info setValue:[NSNumber numberWithBool:NO] forKey:@"endorsements"];
        }
    ...
    if ([HelperSharedManager isCheckNotNULL:usr.correctiveLensRequired]) {
        if ([usr.correctiveLensRequired isEqual:@"yes"]) {
            [info setValue:[NSNumber numberWithBool:YES] forKey:@"corrective_lense_required"];
        } else {
            [info setValue:[NSNumber numberWithBool:NO] forKey:@"corrective_lense_required"];
        }
    ...
    NSString *url = @"students/register/";
        
        if ([HelperSharedManager isCheckNotNULL:self.userItemType]) {
            if ([self.userItemType isEqual:kUserTypeStudent]) {
                [studentParams setValue:[NSNumber numberWithBool:YES] forKey:@"is_student"];
                [studentParams setValue:[NSNumber numberWithBool:NO] forKey:@"is_instructor"];
                
                url = @"students/register/";
            } else {
                [studentParams setValue:[NSNumber numberWithBool:NO] forKey:@"is_student"];
                [studentParams setValue:[NSNumber numberWithBool:YES] forKey:@"is_instructor"];
    ...
    [[ServerManager sharedManager] postUser:token params:studentParams withUrl:url success:^(id responseObject) {
...
-(Aggregate*)aggregate {
    ...
    NSLog(@"UserItemType %@", self.userItemType);
    NSLog(@"User Type %@", self.user.itemType);
