...
-(void)loadPrevInstructor{
    NSString *prevInstructorRecordId = [Settings getSetting:CSD_PREV_INSTRUCTOR_ID];
    ...
    self.instructor = (User*)[instrAgg getObjectWithUserId:prevInstructorRecordId];
    if (saveCurrentUser && self.instructor.userId.length) {
//        [Settings setSetting:self.instructor.rcoRecordId forKey:CSD_PREV_INSTRUCTOR_ID];
...
-(void)loadPrevStudent{
    ...
    if ([prevStudent longLongValue]) {
        ...
        self.student = (User*)[studentAggregate getAnyUserWithEmail:self.student.email];
    }
...
//    self.student = (User*)[studentAggregate getAnyUserWithEmail:self.student.email];
    ...
    if (self.student.userId) {
        ...
        NSString *uId = [NSString stringWithFormat:@"%@/", self.student.userId];
        
        NSString *token = [Settings getSetting:USER_ACCESS_TOKEN];
        
        [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    
        [[ServerManager sharedManager] getUsersDetails:token withType:@"students" withId:uId success:^(id responseObject) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ([HelperSharedManager isCheckNotNULL:info]) {
                    ...
                    if ([HelperSharedManager isCheckNotNULL:[info valueForKey:@"employee_number"]]) {
                        [self.values setObject:[info valueForKey:@"employee_number"] forKey:@"employeeId"];
                    }
                    ...
                    if ([HelperSharedManager isCheckNotNULL:company]) {
                        
                        NSString *companyName = [company objectForKey:@"name"];
                        [self.values setObject:companyName forKey:@"company"];
                        [self loadCompanyWithName:companyName];
                    }
                    ...
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"driver_license_expire_date"]]) {
                        ...
                        [self.values setObject:dateFromString forKey:@"driverLicenseExpirationDate"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"driver_license_number"]]) {
                        [self.values setObject:[info objectForKey:@"driver_license_number"] forKey:@"driverLicenseNumber"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"driver_license_state"]]) {
                        [self.values setObject:[info objectForKey:@"driver_license_state"] forKey:@"driverLicenseState"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"driver_license_class"]]) {
                        [self.values setObject:[info objectForKey:@"driver_license_class"] forKey:Key_Class];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"start_time"]]) {
                        [self.values setObject:[info objectForKey:@"start_time"] forKey:@"startTime"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"end_time"]]) {
                        
                        NSString *dateString = [info objectForKey:@"end_time"];
                        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
//                        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
                        [dateFormatter setDateFormat:@"HH:mm:ss"];
                        NSDate *dateFromString = [dateFormatter dateFromString:dateString];
                        
                        self.testInfo.endDateTime = dateFromString;
//                        [self.values setObject:dateFromString forKey:KeyEndDateTime];
                        
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"corrective_lense_required"]]) {
                        [self.values setObject:[info objectForKey:@"corrective_lense_required"] forKey:@"correctiveLensRequired"];
                    }
                    
                    if (self.student.userId) {
                        [self.values setObject:self.student.userId forKey:@"userId"];
                        

                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"location"]]) {
                        [self.values setObject:[info objectForKey:@"location"] forKey:@"evaluationLocation"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"restrictions"]]) {
                        [self.values setObject:[info objectForKey:@"restrictions"] forKey:@"restrictions"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"comments"]]) {
                        [self.values setObject:[info objectForKey:@"comments"] forKey:@"comments"];
                    }
                    
                    
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"equipement"]]) {
                        [self.values setObject:[info objectForKey:@"equipement"] forKey:@"trainingEquipment"];
                    }
//
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"pre_post_trip"]]) {
                        [self.values setObject:[info objectForKey:@"pre_post_trip"] forKey:@"trainingPreTrip"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"behind_the_wheel"]]) {
                        [self.values setObject:[info objectForKey:@"behind_the_wheel"] forKey:@"trainingBehindTheWheel"];
                    }
                    ...
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"eye_movement"]]) {
                        [self.values setObject:[info objectForKey:@"eye_movement"] forKey:@"trainingEye"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"safe_work_practice"]]) {
                        [self.values setObject:[info objectForKey:@"safe_work_practice"] forKey:@"trainingSWP"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"production"]]) {
                        [self.values setObject:[info objectForKey:@"production"] forKey:@"trainingProd"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"vehicle_road_test"]]) {
                        [self.values setObject:[info objectForKey:@"vehicle_road_test"] forKey:@"trainingVRT"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"passenger_evacuation"]]) {
                        [self.values setObject:[info objectForKey:@"passenger_evacuation"] forKey:@"trainingPassengerEvacuation"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"passenger_pre_trip"]]) {
                        [self.values setObject:[info objectForKey:@"passenger_pre_trip"] forKey:@"trainingPassengerPreTrip"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"new_hire"]]) {
                        [self.values setObject:[info objectForKey:@"new_hire"] forKey:@"trainingNewHire"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"near_miss"]]) {
                        [self.values setObject:[info objectForKey:@"near_miss"] forKey:@"trainingNearMiss"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"incident_follow_up"]]) {
                        [self.values setObject:[info objectForKey:@"incident_follow_up"] forKey:@"trainingIncidentFollowUp"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"change_job"]]) {
                        [self.values setObject:[info objectForKey:@"change_job"] forKey:@"trainingChangeJob"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"change_of_equipment"]]) {
                        [self.values setObject:[info objectForKey:@"change_of_equipment"] forKey:@"trainingChangeOfEquipment"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"annual"]]) {
                        [self.values setObject:[info objectForKey:@"annual"] forKey:@"trainingAnnual"];
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"injury_date"]]) {
                        
                        
                        NSString *dateString = [info objectForKey:@"injury_date"];
                        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
                        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
                        NSDate *dateFromString = [dateFormatter dateFromString:dateString];
                        [self.values setObject:dateFromString forKey:@"trainingInjuryDate"];
                        
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"accident_date"]]) {
                        
                        
                        NSString *dateString = [info objectForKey:@"accident_date"];
                        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
                        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
                        NSDate *dateFromString = [dateFormatter dateFromString:dateString];
                        [self.values setObject:dateFromString forKey:@"trainingAccidentDate"];
                        
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"taw_start_date"]]) {
                        
                        NSString *dateString = [info objectForKey:@"taw_start_date"];
                        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
                        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
                        NSDate *dateFromString = [dateFormatter dateFromString:dateString];
                        [self.values setObject:dateFromString forKey:@"trainingTAWStartDate"];
                        
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"taw_end_date"]]) {
                        
                        NSString *dateString = [info objectForKey:@"taw_end_date"];
                        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
                        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
                        NSDate *dateFromString = [dateFormatter dateFromString:dateString];
                        [self.values setObject:dateFromString forKey:@"trainingTAWEndDate"];
                        
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"lost_time_start_date"]]) {
                        
                        NSString *dateString = [info objectForKey:@"lost_time_start_date"];
                        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
                        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
                        NSDate *dateFromString = [dateFormatter dateFromString:dateString];
                        [self.values setObject:dateFromString forKey:@"trainingLostTimeStartDate"];
                        
                    }
                    
                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"return_work_date"]]) {
                        
                        NSString *dateString = [info objectForKey:@"return_work_date"];
                        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
                        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
                        NSDate *dateFromString = [dateFormatter dateFromString:dateString];
                        [self.values setObject:dateFromString forKey:@"trainingReturnToWorkDate"];
...
//                    if ([HelperSharedManager isCheckNotNULL:[info objectForKey:@"endorsements"]]) {
//                        [self.values setObject:[info objectForKey:@"endorsements"] forKey:@"endorsements"];
//                    }
...
-(void)viewWillDisappear:(BOOL)animated {
    ...
    if ([self.testInfo endDateTime]) {
        ...
        [Settings resetKey:CSD_PREV_STUDENT_ID];
...
-(void)resetTest {
    ...
    [Settings resetKey:CSD_PREV_INSTRUCTOR_ID];
    [Settings resetKey:CSD_PREV_STUDENT];
    [Settings resetKey:CSD_PREV_STUDENT_ID];
    [[DataRepository sharedInstance] resetTrainingTestInfo];
...
-(void)createRecord:(NSNumber*)isAutoSave {
    ...
    if ([HelperSharedManager isCheckNotNULL: self.company.companyId]) {
        [dict setValue:self.company.companyId forKey:@"company"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"correctiveLensRequired"]]) {
        [dict setValue:[self.values objectForKey:@"correctiveLensRequired"] forKey:@"corrective_lense_required"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"driverLicenseClass"]]) {
        [dict setValue:[self.values objectForKey:@"driverLicenseClass"] forKey:@"driver_license_class"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [[self aggregate] rcoDateToString:[self.values objectForKey:@"driverLicenseExpirationDate"]]]) {
        [dict setValue:[[self aggregate] rcoDateToString:[self.values objectForKey:@"driverLicenseExpirationDate"]] forKey:@"driver_license_expire_date"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"driverLicenseNumber"]]) {
        [dict setValue:[self.values objectForKey:@"driverLicenseNumber"] forKey:@"driver_license_number"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"driverLicenseState"]]) {
        [dict setValue:[self.values objectForKey:@"driverLicenseState"] forKey:@"driver_license_state"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: self.instructor.userId]) {
        [dict setValue:self.instructor.userId forKey:@"instructor"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingAnnual"]]) {
        [dict setValue:[self.values objectForKey:@"trainingAnnual"] forKey:@"annual"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingBehindTheWheel"]]) {
        [dict setValue:[self.values objectForKey:@"trainingBehindTheWheel"] forKey:@"behind_the_wheel"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingChangeJob"]]) {
        [dict setValue:[self.values objectForKey:@"trainingChangeJob"] forKey:@"change_job"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingChangeOfEquipment"]]) {
        [dict setValue:[self.values objectForKey:@"trainingChangeOfEquipment"] forKey:@"change_of_equipment"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingEquipment"]]) {
        [dict setValue:[self.values objectForKey:@"trainingEquipment"] forKey:@"equipement"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingEye"]]) {
        [dict setValue:[self.values objectForKey:@"trainingEye"] forKey:@"eye_movement"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingIncidentFollowUp"]]) {
        [dict setValue:[self.values objectForKey:@"trainingIncidentFollowUp"] forKey:@"incident_follow_up"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [[self aggregate] rcoDateToString:[self.values objectForKey:@"trainingInjuryDate"]]]) {
        [dict setValue:[[self aggregate] rcoDateToString:[self.values objectForKey:@"trainingInjuryDate"]] forKey:@"injury_date"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingNearMiss"]]) {
        [dict setValue:[self.values objectForKey:@"trainingNearMiss"] forKey:@"near_miss"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingNewHire"]]) {
        [dict setValue:[self.values objectForKey:@"trainingNewHire"] forKey:@"new_hire"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingPassengerEvacuation"]]) {
        [dict setValue:[self.values objectForKey:@"trainingPassengerEvacuation"] forKey:@"passenger_evacuation"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingPassengerPreTrip"]]) {
        [dict setValue:[self.values objectForKey:@"trainingPassengerPreTrip"] forKey:@"passenger_pre_trip"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingPreTrip"]]) {
        [dict setValue:[self.values objectForKey:@"trainingPreTrip"] forKey:@"pre_post_trip"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingProd"]]) {
        [dict setValue:[self.values objectForKey:@"trainingProd"] forKey:@"production"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingSWP"]]) {
        [dict setValue:[self.values objectForKey:@"trainingSWP"] forKey:@"safe_work_practice"];
    }
    
    if ([HelperSharedManager isCheckNotNULL: [self.values objectForKey:@"trainingVRT"]]) {
        [dict setValue:[self.values objectForKey:@"trainingVRT"] forKey:@"vehicle_road_test"];
    }
   
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"evaluationLocation"]]) {
        [dict setValue:[self.values objectForKey:@"evaluationLocation"] forKey:@"location"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"restrictions"]]) {
        [dict setObject:[self.values objectForKey:@"restrictions"] forKey:@"restrictions"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"comments"]]) {
        [dict setObject:[self.values objectForKey:@"comments"] forKey:@"comments"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"trainingAccidentDate"]]) {
        NSDate *date = [self.values objectForKey:@"trainingAccidentDate"];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSString *dateString = [dateFormatter stringFromDate:date];
        
        [dict setObject:dateString forKey:@"accident_date"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"trainingTAWStartDate"]]) {
        NSDate *date = [self.values objectForKey:@"trainingTAWStartDate"];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSString *dateString = [dateFormatter stringFromDate:date];
        
        [dict setObject:dateString forKey:@"taw_start_date"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"trainingTAWEndDate"]]) {
        NSDate *date = [self.values objectForKey:@"trainingTAWEndDate"];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSString *dateString = [dateFormatter stringFromDate:date];
        
        [dict setObject:dateString forKey:@"taw_end_date"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"trainingLostTimeStartDate"]]) {
        NSDate *date = [self.values objectForKey:@"trainingLostTimeStartDate"];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSString *dateString = [dateFormatter stringFromDate:date];
        
        [dict setObject:dateString forKey:@"lost_time_start_date"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"trainingReturnToWorkDate"]]) {
        NSDate *date = [self.values objectForKey:@"trainingReturnToWorkDate"];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSString *dateString = [dateFormatter stringFromDate:date];
        
        [dict setObject:dateString forKey:@"return_work_date"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"startDateTime"]]) {
        NSDate *date = [self.values objectForKey:@"startDateTime"];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"HH:mm:ss"];
        NSString *dateString = [dateFormatter stringFromDate:date];
        
        [dict setObject:dateString forKey:@"start_time"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:[self.values objectForKey:@"endDateTime"]]) {
        NSDate *date = [self.values objectForKey:@"endDateTime"];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"HH:mm:ss"];
        NSString *dateString = [dateFormatter stringFromDate:date];
        
        [dict setObject:dateString forKey:@"end_time"];
    }
    ...
    NSString *url = [NSString stringWithFormat:@"students/%@/", self.student.userId];
    NSString *token = [Settings getSetting:USER_ACCESS_TOKEN];
    [[ServerManager sharedManager] patchRequest:token withUrl:url withDict:info success:^(id responseObject) {
...
-(NSString*)validateInputs {
    ...
    for (NSString *key in self.requiredFields) {
        ...
        NSLog(@"Key: %@ --- Value: %@", key, [self.values objectForKey:key]);
