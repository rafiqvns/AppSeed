...
-(void)createOrUpdateRecord {
    ...
    if ([HelperSharedManager isCheckNotNULL:self.company.companyId]) {
        [dict setValue:self.instructor.userId forKey:@"company"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue:self.instructor.userId forKey:@"student"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue:[self.values objectForKey:@"powerUnit"] forKey:@"power_unit_number"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"transmissionType"] forKey:@"transmission_type"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"vehicleType"] forKey:@"vehicle_type"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"vehicleTypeOther"] forKey:@"vehicle_type_other"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"vehicleManufacturer"] forKey:@"vehicle_manufacturer"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"vehicleManufacturerOther"] forKey:@"vehicle_manufacturer_other"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"singleTrailerLength"] forKey:@"single_trailer_length"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"singleTrailerLengthOther"] forKey:@"single_trailer_length_other"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"lVCType"] forKey:@"combination_vehicles"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"lVCTypeOther"] forKey:@"combinations_vehicles_other"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"lVCLength"] forKey:@"combination_vehicles_length"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"lVCLengthOther"] forKey:@"combination_vehicles_length_other"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"dollyGearType"] forKey:@"dolly_or_gear_type"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"dollyGearTypeOther"] forKey:@"dolly_or_gear_type_other"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"trailer1Number"] forKey:@"trailer_1"];
    }
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"trailer2Number"] forKey:@"trailer_2"];
    }
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"trailer3Number"] forKey:@"trailer_3"];
    }
    
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"dolly1Number"] forKey:@"dolly_1"];
    }
    if ([HelperSharedManager isCheckNotNULL:self.student.userId]) {
        [dict setValue: [self.values objectForKey:@"dolly2Number"] forKey:@"dolly_2"];
    }
    ...
    NSString *url = [NSString stringWithFormat:@"students/%@/equip/", self.student.userId];
