...
-(void)addDefaultLogin {
    ...
    NSString *server = [NSString stringWithFormat:@"https://%@", CSD_SERVER_DEV];

    [Settings switchToUser:@"csdtrainer" //@"csdtrainer"
              withPassword:@"@20803" //@"@20803"
    ...
    [_loginParameters setValue:@"@20803" forKey:kPassword];
    [_loginParameters setValue:@"csdtrainer" forKey:kLogin];
...
-(void) loadSettings {
    ...
    [_loginParameters setObject:curServer forKey:kURL];
...
- (void)connect {
    ...
    if ( ![[DataRepository sharedInstance] isNetworkReachable] || [[DataRepository sharedInstance] workOffline]) {
        ...
        [[ServerManager sharedManager] authenticateWitUsername:currentUserName password:currentPassword success:^(id responseObject) {
            ...
            NSString *accessToken = [response objectForKey:kAccess];
            if ([HelperSharedManager isCheckNotNULL:accessToken]) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSUserDefaults standardUserDefaults] setValue:accessToken forKey:kSavedAccessToken];
...
- (void)requestFinished:(id )request {
    ...
    if ([request isKindOfClass:[NSData class]]) {
        ...
        errorStatusCode = [(NSDictionary*)request objectForKey:ERROR_STATUS_CODE];
    } else {
