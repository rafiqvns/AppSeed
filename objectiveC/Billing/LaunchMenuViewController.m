...
-(void)loadUsers {
    ...
    controller.companies = self.companies;
    controller.instructors = self.instructors;
    [self launchChildController:controller];
}
...
-(Aggregate*)aggregate {
    return [[DataRepository sharedInstance] getAggregateForClass:kUserType];
}

#pragma mark: - Get students list
...
- (void)getStudents {
    ...
    [[ServerManager sharedManager] getStudents:token success:^(id responseObject) {
        ...
        if ([HelperSharedManager isCheckNotNULL:resp]) {
            ...
            if ([HelperSharedManager isCheckNotNULL:students]) {
                ...
                self.students = [NSMutableArray arrayWithArray:[students sortedArrayUsingDescriptors:[NSArray arrayWithObject:sd]]];
                
                
                for( NSDictionary * studentDict in self.students) {
                    
                    [self createNewStudent:studentDict];
    ...
    [[ServerManager sharedManager] getCompanies:token success:^(id responseObject) {
        ...
        if ([HelperSharedManager isCheckNotNULL:resp]) {
            ...
            if ([HelperSharedManager isCheckNotNULL:companies]) {
                ...
                self.companies = [NSMutableArray arrayWithArray:[companies sortedArrayUsingDescriptors:[NSArray arrayWithObject:sd]]];
                
                TrainingCompanyAggregate *aggregate = (TrainingCompanyAggregate*)[[DataRepository sharedInstance] getAggregateForClass:@"TrainingCompany"];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    for( NSDictionary * companyDict in self.companies) {
                        
                        NSString *name = [companyDict valueForKey:@"name"];
                        TrainingCompany *company = [aggregate getTrainingCompanyWithName:name];
                        
                        
                        
                        if (![HelperSharedManager isCheckNotNULL:company]) { // Company does not exists
                            
                            
                            [aggregate createCompanyWithDict:companyDict];
                            NSLog(@"Comapny created ");
                        } else {
                            NSLog(@"Comapny already exists ");
                        }
...
- (void)createNewStudent: (NSDictionary *) student {
    
    NSDictionary *info = [student valueForKey:@"info"];
    
    
    TrainingStudent *agg1 = (TrainingStudent*)[[DataRepository sharedInstance] getAggregateForClass:@"TrainingStudent"];
    
    UserAggregate *agg = (UserAggregate*)[[DataRepository sharedInstance] getAggregateForClass:@"TrainingStudent"];
    
//    UserAggregate *agg = (UserAggregate*)[[DataRepository sharedInstance] getAggregateForClass:kUserTypeStudent];
    ...
    if (!user) {
        ...
        User *usr = (User*)[[self aggregate] createNewObject];
        usr.dateTime = [NSDate date];
        usr.itemType = kUserType;
        usr.firstname = [student objectForKey:@"last_name"];
        usr.surname = [student objectForKey:@"last_name"];
        usr.email = [student objectForKey:@"email"];
        usr.rcoRecordId = [student objectForKey:@"id"];
        usr.userType = kUserTypeStudent;
        usr.homePhoneNumber = [student objectForKey:@"home_phone_number"];
        usr.workPhoneNumber = [student objectForKey:@"work_phone_number"];
        usr.mobilePhoneNumber = [student objectForKey:@"mobile_number"];
        usr.phone = [student objectForKey:@"phone"];
        usr.sendGPSLocation = [student objectForKey:@"send_gps_location"];
        usr.dateOfBirth = [student objectForKey:@"date_of_birth"];
        usr.sex = [student objectForKey:@"sex"];
        usr.company = [company valueForKey:@"name"];
    //    usr.companyId = [company valueForKey:@"id"];
...
-(BOOL)exist: (NSString *)entityName context : (NSManagedObjectContext*) withContext searchString : (NSString *) str searchField : (NSString *) searchField
{
    
    NSEntityDescription *entity = [NSEntityDescription entityForName:entityName inManagedObjectContext:withContext];

    NSFetchRequest *request = [[NSFetchRequest alloc] init];
    [request setEntity:entity];
    [request setFetchLimit:1];
    [request setPredicate:[NSPredicate predicateWithFormat:@"%@ == %@", searchField, str]];

    NSError *error = nil;
    NSUInteger count = [withContext countForFetchRequest:request error:&error];
